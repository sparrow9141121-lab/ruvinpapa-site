...
- **초저녁야담 섹션** — 자매 채널 [초저녁야담](https://www.youtube.com/channel/UCoEeRkwnzuHWwGnOQz1RUFg)
  최신 에피소드 3개 (쇼츠 제외, 제목 뒤 `/ 야담 옛날이야기 …` 태그는 떼고 표시).
  이 채널을 못 읽어도 본 채널 갱신은 그대로 진행되고, 야담 목록만 기존 것을 유지합니다.
- 히어로 카드의 세 줄 요약, 마지막 갱신 시각

### 조회수를 읽는 방법 두 가지

1. **공식 API** — 환경변수 `YOUTUBE_API_KEY` 가 있으면 YouTube Data API v3 로 읽습니다.
   깃허브에서도 잘 동작하므로 이 방법을 권합니다. (하루 할당량 10,000 유닛 중 몇 유닛만 씁니다)
2. **일반 접속** — 키가 없으면 영상 페이지에서 직접 읽습니다. 내 컴퓨터에서는 잘 되지만,
   깃허브 서버에서는 유튜브가 막는 경우가 있습니다.

둘 다 실패하면 **대표 영상을 지금 올라가 있는 것 그대로 두고** 최신 목록과 대표 글만
갱신합니다. 그래서 매일 돌아도 대표가 제멋대로 뒤바뀌지 않습니다.

#### API 키 등록하기

- 깃허브: 저장소 **Settings → Secrets and variables → Actions → New repository secret**
  이름은 `YOUTUBE_API_KEY`
- 내 컴퓨터에서 쓰려면 실행 전에 환경변수로 넣어 주면 됩니다
  (`set YOUTUBE_API_KEY=...` 뒤에 `python update-content.py`)

키는 저장소 파일에 절대 적지 마세요. Secrets 에만 넣습니다.
